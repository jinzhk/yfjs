...
(function (factory) {
	...
	var Spinbox = function (element, options) {
		...
		this.$element.on('focusin.bs.spinbox', this.$input, $.proxy(this.changeFlag, this));
		...
		if (this.options.hold) {
			this.$element.on('mousedown.bs.spinbox', '.spinbox-up', $.proxy(function () {
				this.startSpin(true);
			}, this));
			this.$element.on('mouseup.bs.spinbox', '.spinbox-up, .spinbox-down', $.proxy(this.stopSpin, this));
			this.$element.on('mouseout.bs.spinbox', '.spinbox-up, .spinbox-down', $.proxy(this.stopSpin, this));
			this.$element.on('mousedown.bs.spinbox', '.spinbox-down', $.proxy(function () {
				this.startSpin(false);
			}, this));
		} else {
			this.$element.on('click.bs.spinbox', '.spinbox-up', $.proxy(function () {
				this.step(true);
			}, this));
			this.$element.on('click.bs.spinbox', '.spinbox-down', $.proxy(function () {
				this.step(false);
			}, this));
		}
	...
	Spinbox.prototype = {
		...
		triggerChangedEvent: function () {
			...
			this.$element.trigger('changed.bs.spinbox', this.output(currentValue, false));// no DOM update
		},
	...
	$.fn.spinbox = function (option) {
		...
		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('bs.spinbox');
			var options = typeof option === 'object' && option;

			if (!data) {
				$this.data('bs.spinbox', (data = new Spinbox(this, options)));
			}
