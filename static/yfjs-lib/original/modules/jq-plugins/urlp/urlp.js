 ...
 *
 *	$("a").urlp("user");
 *	$("a").urlp("user", "username");
 *
 *	$("a").urlp("password");
 *	$("a").urlp("password", "password");
 *
 *	$("a").urlp("host");
 ...
 *	      .urlp("filter", "user", "=", "user")
 *	      .urlp("filter", "password", "=", "password")
 ...
 *	url = $.urlp("http://www.example.com:80/path/file.name?param1=value1#fragment");
 *
 ...
 *	- A URL parser interface returned from $.urlp().
 *
 **/
...
define(['jquery'], function ( $ )
    {
        ...
        var initialiseElementText = function ( )
        {
            ...
            if ( this.get ( 0 ) != $( document ).get ( 0 ) && this.attr ( "href" ) == null && this.attr ( "src" ) == null )
            {
                ...
                if ( this.html ( ) != null && this.hasClass ( "urlp-span" ) == false )
                {
                    var urls = [ ];
                    ...
                    for ( var i = 0; i < urls.length; i++ )
                    {
                        ...
                        else
                        {
                            ...
                            html = html.replace ( "$" + i, "<a href=\"[url]\" class=\"urlp-no-watch\">[url]</a>".replace ( /\[url\]/g, urls [ i ] ) );
                        }
                    }
        ...
        var methods =
        {
             ...
             *	$(document).urlp("url", "http://www.google.com");
             *
             *	// Replace all anchor tags with the google URL!
             *	$("a").urlp("url", "http://www.google.com");
             *
             *	// Output the documents URL object
             *	console.log($(document).urlp("url"));
             *
             *	// Output the documents URL string
             *	console.log($(document).urlp("url").toString());
             *
             *	(end code)
             *
             **/
             ...
             *	console.log($(document).urlp("path"));
             *
             *	// Output the documents URL path string
             *	console.log($(document).urlp("path").toString());
             *
             *	(end code)
             *
             **/
             ...
             *	$("a").urlp("proxy", "http://www.example.com", "url");
             *
             *	(end code)
             *
             **/
             ...
             *	// THIS WILL NOT WORK!:
             *	$("a").urlp("watch").
             *	      .urlp("filter", "host", "=", "www.facebook.com").
             *	      .urlp("query",{"found":"facebook"});
             *
             *	// This call will fail, as the "a" selector is now watched.
             *	$("a").urlp("watch").
             *	      .urlp("filter", "host", "=", "www.twitter.com")
             *	      .urlp("query",{"found":"twitter"});
             *
             *	// THIS WILL WORK!:
             *	$("a:not(facebook)").urlp("watch").
             *	                    .urlp("filter", "host", "=", "www.facebook.com")
             *	                    .urlp("query",{"found":"facebook"});
             *
             *	$("a:not(twitter)").urlp("watch")
             *	                   .urlp("filter", "host", "=", "www.twitter.com")
             *	                   .urlp("query",{"found":"twitter"});
             *
             *	(end code)
                ...
                if ( selectorCallStack [ currentSelector ] == null )
                {
                    ...
                    (
                        ...
                        function DOMListener ( event )
                        {
                            ...
                            /* Ensure we have a target to modify, and that we are allowed to watch it (the ".urlp-no-watch" class is present on elements created in initialiseElementText()). */

                            if ( target.length > 0 && target.is ( ".urlp-no-watch" ) == false )
                            {
                                var filtered = false;
        ...
        $.urlp = function ( url )
        {
            ...
            return {
                ...
                watch : function ( callback )
                {
                    ...
                    return $( "[href*=\"" + host + "\"],[src*=\"" + host + "\"]" ).urlp ( "watch", callback );
                },
                unwatch : function ( )
                {
                    /* Get the current host name */
                    ...
                    return $( "[href*=\"" + host + "\"],[src*=\"" + host + "\"]" ).urlp ( "unwatch" );
                }
            };
        };
    }
