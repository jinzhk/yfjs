 ...
 * =========================================================
 *
 * Updated by jinzhk @ 2015-05-07
 *
 * 1、模块调用方式添加commonjs写法
 * 2、提示信息默认使用中文，英文提示移至locale目录下
 *
 * Updated by jinzhk @ 2016-05-16
 *
 * 1、resetForm方法添加了resetData参数，用以设置是否同时重置表单数据
 *
 * =========================================================
 */
...
$.extend( $.validator, {
	...
	prototype: {
		...
		resetForm: function(resetData) {
			if ( resetData !== false && $.fn.resetForm ) {
			...
			$.ajax( $.extend( true, {
				...
				success: function( response ) {
					...
					if ( typeof response === 'object' ) {
						valid = true;
						var key = param.key ?  param.key : element.name;
						if (param.id) {
							var id = param.id;
						}
						response = param.dataSrc ? response[ param.dataSrc ] : response;
						$.each( response, function( i,item ) {
							if ( item[ key ] == value && (id !== item.id || !id) ) {
								valid = false;
								return false;
							}
						} );
