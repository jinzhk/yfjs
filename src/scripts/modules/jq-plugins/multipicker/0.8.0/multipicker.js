...
define(['jquery'], function($) {
    ...
    MultiPicker.prototype.bindEvents = function() {
        this.$element.off('created.multipicker').on('created.multipicker', $.proxy(function(e) {
            if (typeof this.options.oncreated === "function") {
                this.options.oncreated.apply(this, arguments);
            }
        }, this));
        this.$element.off('changed.multipicker').on('changed.multipicker', $.proxy(function(e) {
        ...
        this.$element.off('selected.multipicker').on('selected.multipicker', $.proxy(function(e) {
            if (typeof this.options.onselected === "function") {
                e.data = $(e.relatedTarget).data(MultiPicker.DATA_KEY).originalData;
                this.options.onselected.apply(this, arguments);
            }
        }, this));
        this.$element.off('deselected.multipicker').on('deselected.multipicker', $.proxy(function(e) {
    ...
    MultiPicker.prototype.create = function() {
        ...
        this.$toColumn = $(".multipicker-column-to", this.$element);
        this.$toList = $(".multipicker-column-list", this.$toColumn);

        if (this.$toColumn[0] && this.$toColumn[0].className && this.$toColumn[0].className.indexOf("col-") > -1) {
            this.$element.addClass("row");
        } else if (this.$toColumn.css('float') != "none") {
            this.$element.addClass("clearfix");
        }

        this.createMoreItem(MultiPicker.TYPE_SELECTED);

        this.$fromColumn = $(".multipicker-column-from", this.$element);
        this.$fromList = $(".multipicker-column-list", this.$fromColumn);

        if (this.$fromColumn[0] && this.$fromColumn[0].className && this.$fromColumn[0].className.indexOf("col-") > -1) {
            this.$element.addClass("row");
        } else if (this.$fromColumn.css('float') != "none") {
            this.$element.addClass("clearfix");
        }
        ...
        this.$element.trigger($.Event("created.multipicker"));
    ...
    MultiPicker.prototype.createMoreItem = function(type) {
        if (type === MultiPicker.TYPE_SELECTED && this.options.toColumn.loadMore) {
            if (this.$toList.children(".multipicker-column-more").length <= 0) {
                this.$toList.append('<li class="multipicker-column-more"><a href="javascript:;">加载更多</a></li>');
            }
            ...
            if (this.$fromList.children(".multipicker-column-more").length <= 0) {
                this.$fromList.append('<li class="multipicker-column-more"><a href="javascript:;">加载更多</a></li>');
            }
        }
    ...
    MultiPicker.prototype.handleEvents = function() {
        ...
        var doAddItem = function($item) {
            ...
            $item.remove();
            self.$element.trigger($.Event("selected.multipicker", {relatedTarget: $newItem[0]}));
            if (self.isChanged()) {
                self.$element.trigger($.Event("changed.multipicker", {relatedTarget: $newItem[0]}));
            }
        };
        var doDeleteItem = function($item) {
            ...
            $item.remove();
            self.$element.trigger($.Event("deselected.multipicker", {relatedTarget: $newItem[0]}));
            if (self.isChanged()) {
                self.$element.trigger($.Event("changed.multipicker", {relatedTarget: $newItem[0]}));
            }
        };
