 ...
 * Fixed: 修复了因单元格边框宽度的有无导致左右两侧固定列定位不准确的问题
 */
...
FixedColumns.prototype = /** @lends FixedColumns.prototype */{
	...
	"_fnConstruct": function ( oInit )
	{
        ...
        $(window).off('resize.DTFC-'+oSettings.sInstance)
            .on('resize.DTFC-'+oSettings.sInstance, {that: that}, function (evt) {
                var $this = $(this);
                try {
                    var that = evt.data.that,
                        oSetting = that.s.dt;
                    if (!document.getElementById(oSetting.sInstance)) {
                        $this.unbind('resize.DTFC-'+oSetting.sInstance);
                        if (evt.data) {
                            delete evt.data.that;
                        }
                    } else {
        ...
        $(window).off('destroy.dt')
            .on('destroy.dt', function(e, settings) {
                settings = settings || {};
                if (settings.sInstance != null) {
                    $(window).off( 'resize.DTFC-' + settings.sInstance );
                }
            });
		...
		jqTable
			...
			.on( 'destroy.dt.DTFC', function () {
				...
				$(window).off( 'resize.DTFC-' + that.s.dt.sInstance );
	...
	"_fnGridLayout": function ()
	{
        ...
        var $cell = $("tr:eq(0)", this.s.dt.nTable).children("th,td"),
            cellLeftBW = 0, cellRightBW = 0;
        if ($cell && $cell.length) {
            if ($cell.length > 1) {
                $cell = $cell.eq(1);
            } else {
            ...
            cellLeftBW = parseInt( $cell.css('border-left-width'), 10 );
            cellRightBW = parseInt( $cell.css('border-right-width'), 10 );
        }

		if ( this.s.iLeftColumns > 0 )
		{
            iLeftWidth += cellLeftBW;
...
if ( typeof define === 'function' && define.amd ) {
	define( ['jquery', 'jq/dataTables'], factory );
}
else if ( typeof exports === 'object' ) {
    // Node/CommonJS
    factory( require('jquery'), require('jq/dataTables') );
}
