...
function build(commander,changeFile){
    ...
    pathArr.forEach(function(item,idx){
        ...
        // 站外链接处理
        markedstr  = linkTarget(markedstr);

        // ejs 模板生成HTML
        html = file.ejs(template + '/layout.ejs',{
            title: pkg.title || pkg.name,//项目工程名字
            index:isIndex,//是否为首页
            pkg:pkg,
    ...
    copy( template +'/source/', todir+'static',{
        filter:function(_file){
            var relativePath = path.normalize(template +'/source/');
            var posRelative = _file.indexOf(relativePath);
            if (posRelative > -1) {
                _file = _file.substring(relativePath.length);
            }
            if (/\.(js)$/.test(_file) && /^js(\\|\/)/.test(_file)) {
                return false;
            }
            if(!/\.(styl)$/.test(_file)) {
                return true;
    ...
    // 压缩 js 文件
    uglify(template+'/source/js/',function(result,_filename,_extname){
        if (result != null) {
            if (typeof result == "string" || result.code == null) return log(result);
            file.write(todir+ 'static/js/'+_filename+'.js',result.code);
        }
    });
